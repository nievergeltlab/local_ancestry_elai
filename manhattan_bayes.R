...
ManhattanPlot_bayes <- function(chr, pos, pvalue, snp, genomesig = 0, genomesug = 0,photoname = "Manhattan Plot", outname = "ManhattanPlot_AJS", colors = c(rgb(230,185,184,max=255),"black"), sigcolor = "darkred", sugcolor = "indianred", ncex = .5, plotsymbol = 19, sugcex = 1, sigcex = 1, nonautosome = c(23,24,25,26),xlabel = "Chromosomal Positions",ylabel = "-log10(p-value)", pdf = "TRUE",topsnplabels = "FALSE", pvalue_miss = "NA") {
	...
	for (i in 1:length(pvalue)){ 
		if (genomesig == 0) {
			if (genomesug == 0) {
				...
				if (pvalue[i] > genomesug) {
					ptcex[i] <- sugcex
				}else (ptcex[i] <- ncex)
			}
		}else {
			if (pvalue[i] > genomesig) {
				ptcex[i] <- sigcex
			}else {
				if (genomesug == 0) {
					ptcex[i] <- ncex
	...
	plot(pos + chrominfo[,3][match(chr,chrominfo[,1])],pvalue, col = chrominfo[,6][match(chr,chrominfo[,1])],cex = ptcex, pch = plotsymbol, ann = F, xaxt = "n", xlim = c(0,1.015*xmax), ylim = c(0, 1),cex.axis=1.45, cex.lab=1.5)
